...
class JobApplicationForm(forms.ModelForm):
    ...
    phone = forms.IntegerField(label='Email', widget=forms.NumberInput(attrs={'id':' phoneNumber', 'class': 'form-control form-control-lg', 'type' : 'tel'}))
    resume = forms.FileField(label='File', widget=forms.FileInput(attrs={'type': 'file', 'name': 'resume', 'accept' : '.pdf,.doc,.docx', 'id' : 'resume'}))
    birthday = forms.DateField(label='Date', widget=forms.DateInput(attrs={'id': 'birthdayDate', 'type': 'date', 'class': 'form-control form-control-lg',}))
    ...
    class Meta:
        ...
        fields = ['first_name', 'last_name', 'email', 'phone', 'birthday', 'resume', 'gender']